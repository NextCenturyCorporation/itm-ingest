...
VERBOSE = False # Request quieter output
...
def delete_bad_adms(mongo_db, collection_name: str, target_field: str, adm_field: str, adms: list) -> list:
    ...
    for adm in adms:
        if '-MF' not in adm[target_field]:
            adms_to_keep.append(adm) # Keep all AF-PS adms
            continue

        original_adm_name = str(adm.get(adm_field, ''))
        should_delete = False
...
def rename_adms(mongo_db, adms_to_keep: list):
