...
def add_adm_alignment_fields(mongo_db):
...
def main(mongo_db):
    # Add admAlignment and admTarget fields FIRST
    add_adm_alignment_fields(mongo_db)
    ...
    for key, value in special_case_pids.items():
        ...
        if vol_scenario:
            st_sid = (
                requests.post(f"{ST_URL}api/v1/new_session?user_id=default_user")
                .text.replace('"', "")
                .strip()
            )
            probes = []
            ...
            mostLeast = requests.get(
                f"{ST_URL}api/v1/get_ordered_alignment?session_id={st_sid}&kdma_id=PerceivedQuantityOfLivesSaved"
            ).json()
            kdma = requests.get(
                f"{ST_URL}api/v1/api/v1/computed_kdma_profile?session_id={st_sid}"
            ).json()
        ...
        # update exempt status as needed
        special_case_survey = mongo_db["surveyResults"].find_one({"results.pid": value})
        if not special_case_survey:
            continue
            
        res = special_case_survey.get("results", {})
        for page_key, page_data in res.items():
            if "Medic" not in page_key or "vs" in page_key or page_data.get("admAlignment") == "baseline":
                continue

            target = page_data["admTarget"]
            scenario_pattern = "vol" if "vol" in target.lower() else ("IO" if "Ingroup" in target else "MJ")
            
            participant_text_scenario = text_scenarios.find_one(
                {"participantID": value, "scenario_id": {"$regex": scenario_pattern}}
            )

            should_exempt = False
            if not participant_text_scenario:
                should_exempt = True
            else:
                alignment = page_data["admAlignment"]
                index = 0 if alignment == "aligned" else -1 if alignment == "misaligned" else None
                
                if index is None:
                    print(f"Unexpected alignment: {alignment}")
                    continue
                
                if scenario_pattern == "vol":
                    target_at_index = participant_text_scenario["mostLeastAligned"][index]["target"]
                else:
                    target_type = "Moral judgement" if scenario_pattern == "MJ" else "Ingroup Bias"
                    mostLeast = next(
                        (obj.get("response", []) for obj in participant_text_scenario.get("mostLeastAligned", []) 
                        if obj.get("target") == target_type), 
                        None
                    )
                    if mostLeast:
                        target_at_index = list(mostLeast[index].keys())[0]
                        target_at_index = target_at_index[:-1] + "." + target_at_index[-1]
                    else:
                        should_exempt = True
                
                if not should_exempt and target_at_index != target:
                    should_exempt = True
            
            if should_exempt:
                mongo_db["surveyResults"].update_one(
                    {"_id": special_case_survey["_id"], f"results.{page_key}": {"$exists": True}},
                    {"$set": {f"results.{page_key}.admChoiceProcess": "exempt"}}
                )
    ...
    for doc in docs:
        kdma_value = (
            doc.get("kdmas", [{}])[0].get("value") if doc.get("kdmas") else None
        )
    ...
    for doc in docs:
        ...
        probes = [
            {"probe": {"choice": x["choice"], "probe_id": x["probe_id"]}}
            for x in doc["probes"]
        ]
        ...
        adm_delegation_runs.update_one(
            {"_id": doc["_id"]}, {"$set": {"kdmas": kdma, "session_id": sid}}
        )
    ...
    for doc in target_runs_docs:
        ...
        target_runs.update_one({"_id": doc["_id"]}, {"$set": {"session_id": sid}})
    ...
    for survey in survey_results:
        ...
        participant_vol_res = text_scenarios.find_one(
            {"participantID": pid, "scenario_id": {"$regex": "vol-ph1-eval"}}
        )
        ...
        if participant_vol_res:
            ...
            for k in participant_vol_res:
                if (
                    isinstance(participant_vol_res[k], dict)
                    and "questions" in participant_vol_res[k]
                ):
                    if (
                        "probe " + k in participant_vol_res[k]["questions"]
                        and "response"
                        in participant_vol_res[k]["questions"]["probe " + k]
                    ):
                        if (
                            "question_mapping"
                            in participant_vol_res[k]["questions"]["probe " + k]
                        ):
                            response = participant_vol_res[k]["questions"][
                                "probe " + k
                            ]["response"].replace(".", "")
                            mapping = participant_vol_res[k]["questions"]["probe " + k][
                                "question_mapping"
                            ]
                            if response in mapping:
                                text_probes.append(
            ...
            for page_key, page_data in results.items():
                if (
                    ...
                    and page_data.get("scenarioIndex")
                    and "vol" in page_data.get("scenarioIndex", "")
                ):
                    if (
                        page_data.get("pageType") != "comparison"
                        and "vs" not in page_key
                    ):
                            ...
                            comparison_collec.update_many(
                                ...
                                },
                                {
                                    "$set": {
                                        ...
                                        "text_session_id": participant_vol_res[
                                            "serverSessionId"
                                        ],
        ...
        for page_key, page_data in results.items():
            if "vs" in page_key and isinstance(page_data, dict):
                ...
                if (
                    ...
                    filt = (
                        "Ingroup%20Bias"
                        if "Bias" in baseline_target
                        else "Moral%20judgement"
                    )
                    ...
                    if not participant_res:
                        print(
                            f"No participant text scenario found for pid: {pid}, pattern: {scenario_pattern}"
                        )
                        continue
