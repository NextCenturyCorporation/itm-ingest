...
def main(mongo_db):
    ...
    actual_adm_runs = adm_collection.find({'evalNumber': EVAL_NUM, 'synthetic': {'$exists': False}})
    synthetic_aligned_runs = adm_collection.find({'evalNumber': EVAL_NUM, 'synthetic': True, 'evaluation.adm_profile': 'aligned'})
        ...
        for history_entry in actual_adm_run['history']:
            if history_entry['command'] == 'Alignment Target':
                ...
                if synthetic_id not in target_map.keys():
                    ...
                    kdmas: dict = {'affiliation': af, 'merit': mf, 'personal_safety': ps, 'search': ss}
    ...
    def find_target(target_kdmas: list) -> str:
        for id in target_map:
            if target_map[id]['affiliation'] == get_kdma_value(target_kdmas, 'affiliation') and \
                target_map[id]['merit'] == get_kdma_value(target_kdmas, 'merit') and \
                target_map[id]['personal_safety'] == get_kdma_value(target_kdmas, 'personal_safety') and \
                target_map[id]['search'] == get_kdma_value(target_kdmas, 'search'):
                    return id
    ...
    for adm_name in adm_groups:
        for target_id in adm_groups[adm_name]:
            ...
            first_adm = adm_groups[adm_name][target_id][0] # human kdmas will be the same in all ADMs
            target_af = get_kdma_value(first_adm['evaluation']['human_kdmas'], 'affiliation')
            ...
            for pid in matching_pids:
                ...
                for adm in adm_groups[adm_name][target_id]:
                    ...
                    if 'score' in res:
                        ...
                        print(f'Could not get comparison score for (text session {human_session_id}; adm session {adm_session_id})')
                ...
                new_doc['ave_aligned_alignment'] = aligned_alignment_sum / max(1, aligned_alignment_count)
    ...
    for pid in pids:
        ...
        multi_kdmas_4d.update_one(
            {'pid': pid},
            {'$set': {'af_base_ave': af_base_sum / max(1, af_base_count),
