...
def main(mongo_db):
    ...
    for adm in observed_adms_cursor:
        ...
        if "MJ2" in adm["scenario"]:
            ...
            This addresses moral judgement scoring descrepency in scenario MJ2
            '''
            ...
            if "Ingroup Bias" in adm['target']:
                ...
                cleaned_probes = [probe for probe in adm["probes"] if probe.get("probe_id") not in remove_probes]

                matching_adm = full_adms.find_one({"evalNumber": 5, "adm_name": adm["adm_name"], 
                                      "scenario": adm["scenario"], "alignment_target": adm['target']})
                ...
                if matching_adm and 'history' in matching_adm:
                    ...
                    for probe_id in ["Probe 4-B.1-B.1", "Probe 4-B.1", "Probe 4"]:
                        probe_entry = next((entry for entry in matching_adm['history'] 
                              if entry.get('parameters', {}).get('probe_id') == probe_id), None)
                        if probe_entry:
                            probe_choice = probe_entry.get('parameters', {}).get('choice')
                            ...
                            cleaned_probes.append({
                                ...
                                "choice": new_choice,
                                "justification": probe_entry['parameters'].get('justification', '')
                            })
        ...
        if "MJ4" in adm["scenario"] and "Moral judgement" in adm['target']:
            problem_probe_exists = any(probe.get("probe_id") == "Probe 10-A.1" for probe in adm["probes"])
            
            if problem_probe_exists:
                # remove probe 10-A.1
                cleaned_probes = [probe for probe in adm["probes"] if probe.get("probe_id") != "Probe 10-A.1"]

                matching_adm = full_adms.find_one({"evalNumber": 5, "adm_name": adm["adm_name"], "scenario": adm["scenario"], "alignment_target": adm['target']})
                if matching_adm and 'history' in matching_adm:
                    # find response to probe 10
                    probe_10_entry = next((entry for entry in matching_adm['history'] if entry.get('parameters', {}).get('probe_id') == "Probe 10"), None)
                    if probe_10_entry:
                        cleaned_probes.append({
                            "probe_id": "Probe 10",
                            "scenario_id": adm["scenario"],
                            "choice": probe_10_entry['parameters'].get('choice', ''),
                            "justification": probe_10_entry['parameters'].get('justification', '')
                        })
        ...
        if "MJ5" in adm["scenario"]:
            ...
            Everytime an ADM answered Probe 8 with Response 8-A, Probe 8-A.1-A.1 needs to be answered as well.  
            ...
            if "Ingroup Bias" in adm['target']:
                ...
                probe_answer = any(probe.get('probe_id') == 'Probe 8' and probe.get('choice') == 'Response 8-A' 
                                for probe in adm['probes'])
                
                if probe_answer:
                    # Check if they already have a response to Probe 8-A.1-A.1
                    follow_up_probe = any(probe.get('probe_id') == 'Probe 8-A.1-A.1' for probe in adm['probes'])
                    ...
                    if not follow_up_probe:
                        ...
                        if matching_adm and 'history' in matching_adm:
                            ...
                            original_choice = probe.get('parameters', {}).get('choice', '')
                            choice = 'Response 8-A.1-A.1-A' if original_choice.endswith("-A") else "Response 8-A.1-A.1-B"
                            ...
                            cleaned_probes.append({
                                ...
                                "scenario_id": adm["scenario"],
                                "choice": choice,
                                "justification": probe['parameters'].get('justification', '')
                            })
            ...
            Make sure Probe 2-A.1-A.1 or Probe 2-B.1-A.1 answered depedning on Probe 2 response
            '''
            
            if 'Moral judgement' in adm['target']:
                problem_probes = ['Probe 2-B.1', 'Probe 2-A.1']
                has_problem_probes = any(probe.get('probe_id') in problem_probes for probe in adm['probes'])
                
                probe_2_response_B = next((probe for probe in adm['probes'] 
                                        if probe.get('probe_id') == 'Probe 2' and probe.get('choice') == 'Response 2-B'), None)
                probe_2_A1_A1 = next((probe for probe in adm['probes'] 
                                    if probe.get('probe_id') == 'Probe 2-A.1-A.1'), None)
                ...
                if has_problem_probes:
                    cleaned_probes = [probe for probe in cleaned_probes if probe.get('probe_id') not in problem_probes]
                    need_changes = True
                
                if probe_2_response_B and probe_2_A1_A1:
                    cleaned_probes = [probe for probe in cleaned_probes if probe.get('probe_id') != 'Probe 2-A.1-A.1']
                    choice = 'Response 2-B.1-A.1-A' if probe_2_A1_A1.get('choice', '').endswith("A") else 'Response 2-B.1-A.1-B'
...
def run_adm(adm, probes, observed_adms, comparison_collec):
    ...
    ADEPT_URL = config("ADEPT_DRE_URL") if ((eval_num == 4 and not dre_ph1_run) or 
        (eval_num == 5 or eval_num == 6) and ph1_in_dre_server_run) else config('ADEPT_URL')
    
    old_session_id = adm['session_id']
    old_id = adm['_id'] 
    
    # run observed adm with updated probes. new doc in delegationADMRuns
    new_observed_adm = db_utils.mini_adm_run(5, observed_adms, probes, adm['target'], 
                                            adm.get('adm_name', ''), dre_ph1_run, ph1_in_dre_server_run, True)
    ...
    if delete_result.deleted_count > 0:
        print(f"Deleted old ADM document with ID: {old_id}, session_id: {old_session_id}")
    else:
    ...
    for comp in matching_comps:
        ...
        if 'Moral' in adm['target']:
            score = requests.get(f'{ADEPT_URL}api/v1/alignment/compare_sessions_population?session_id_1_or_target_id={human_sid}&session_id_2_or_target_id={new_session_id}&target_pop_id=ADEPT-DryRun-Moral%20judgement-Population-All').json()
        else:
            score = requests.get(f'{ADEPT_URL}api/v1/alignment/compare_sessions_population?session_id_1_or_target_id={human_sid}&session_id_2_or_target_id={new_session_id}&target_pop_id=ADEPT-DryRun-Ingroup%20Bias-Population-All').json()
        
        # replace adm session id and (population) score on comparison document
        comparison_collec.update_one({"_id": comp['_id']}, {"$set": {"score": score["score"], "adm_session_id": new_session_id}})

        # updates distance based score as well
        if comp.get('distance_based_score') is not None:
            if 'Moral' in adm['target']:
                score = requests.get(f'{ADEPT_URL}api/v1/alignment/compare_sessions?session_id_1={human_sid}&session_id_2={new_session_id}&kdma_filter=Moral%20judgement').json()
            else:
                score = requests.get(f'{ADEPT_URL}api/v1/alignment/compare_sessions?session_id_1={human_sid}&session_id_2={new_session_id}&kdma_filter=Ingroup%20Bias').json()
            comparison_collec.update_one({"_id": comp['_id']}, {"$set": {"distance_based_score": score["score"], "adm_session_id": new_session_id}})