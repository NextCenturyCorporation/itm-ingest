...
def main(db):
    '''
    Runs through all adept survey pages for JAN Eval and calculates
    alignment using the dre server
    '''
    send_JAN_to_DRE(db)
    ...
    surveys = survey_collection.find({'results.evalNumber': 6})
    for res in surveys:
        ...
        for pageName in res['results']:
            if 'Medic' in pageName and ' vs ' not in pageName:
                ...
                comparison_params = {'dre_server': True, 'evalNumber': 6, 'adm_type': page['admAlignment'], 'pid': pid, 'adm_author': page['admAuthor'], 'adm_scenario': page['scenarioIndex']}
                ...
                txt_alignment = get_target_score(DRE_URL, txt_id, target)
                ...
                adm_alignment = get_target_score(DRE_URL, adept_sid, target)
                ...
                alignments = {}
                for t in targets:
                    alignments[t] = get_target_score(DRE_URL, txt_id, t)
                ...
                page['dreChoiceProcess'] = choice_process
                page['dreAdmAlignment'] = adm_alignment
                page['dreTxtAlignment'] = txt_alignment
...
def get_target_score(url, session_id, target):
    '''
    Use DRE server for eval 6
    '''
...
def send_JAN_to_DRE(mongoDB, EVAL_NUMBER=6):
    '''
    Populates the DRE server with JAN eval data
    '''
    ...
    # go through text scenarios to find comparison values (text scenarios should have been set up in 053)
    text_scenario_collection = mongoDB['userScenarioResults']
        ...
        for page in survey['results']:
            if 'Medic' in page and ' vs ' not in page:
                ...
                if ('qol' in scenario_id and 'qol' in page_scenario) or ('vol' in scenario_id and 'vol' in page_scenario):
                    ...
                    found_mini_adm = del_adm_runs_collection.find_one({'jan_in_dre_server_run': True, 'target': adm_target, 'evalNumber': EVAL_NUMBER, 'scenario': page_scenario.replace('IO', 'MJ'), 'adm_name': survey['results'][page]['admName']})
