...
class DelegationTool:
                        ...
                        for pat in el.get('patients', []):
                            if 'imgUrl' in pat and len(pat['imgUrl']) > 100:
                                ...
                                for doc in res:
                                    ...
                                    break
                                if not found:
                                    res = self.image_mongo_collection.insert_one({'url': pat['imgUrl'], 'patientIds': [pat['name']], 'description': pat['description'], 'scenario': page['scenarioIndex'], 'mediaType': 'img'})
    ...
    def add_img_to_db(self, img_path, patient_id, description, scenario_index):
        ...
        found = False
        for doc in res:
            self.image_mongo_collection.update_one({'_id': doc['_id']}, {'$push': {'patientIds': patient_id}} )
            return doc['_id']
        if not found:
    ...
    def add_db_medic_to_survey_by_details(self, adm_name, adm_alignment, scenario_writer, environment, append=False):
        ...
        Adds it to the survey at the end of all singleMedics
        '''
        if adm_name not in ['kitware-single-kdma-adm-aligned-no-negatives', 'kitware-single-kdma-adm-baseline', 'kitware-hybrid-kaleido-aligned', 'TAD aligned', 'TAD baseline', 'TAD severity-baseline', 'TAD misaligned']:
            LOGGER.log(LogLevel.WARN, "ADM name must be one of ['kitware-single-kdma-adm-aligned-no-negatives', 'kitware-single-kdma-adm-baseline', 'kitware-hybrid-kaleido-aligned', 'TAD aligned', 'TAD baseline', 'TAD severity-baseline', 'TAD misaligned']. Cannot add medic")
            return
        adm_alignment = adm_alignment.lower()
        if adm_alignment not in ['high', 'low']:
            LOGGER.log(LogLevel.WARN, "ADM Alignment must be either 'high' or 'low'. Cannot add medic.")
            return
        if scenario_writer not in ['Adept', 'SoarTech']:
            LOGGER.log(LogLevel.WARN, "Scenario writer must be either 'Adept' or 'SoarTech'. Cannot add medic.")
            return
        environment = environment[0].upper() + environment[1:].lower()
        if environment not in ['Desert', 'Jungle', 'Urban', 'Submarine']:
            LOGGER.log(LogLevel.WARN, "Environment must be one of ['Desert', 'Jungle', 'Urban', 'Submarine']. Cannot add medic.")
            return
        found_docs = self.medics_mongo_collection.find({"admName": adm_name, "admAlignment": adm_alignment, "scenarioName": scenario_writer + ' ' + environment})
        found_medic = False
        for doc in found_docs:
            found_medic = True
            if not append:
...
def one_time_db_initialization():
    ...
    tool.add_img_to_db(os.path.join(img_dir, 'ad_desert_local.png'), 'local_soldier_1', 'Male, mid-30s; local military uniform.  Tried to break up fight between US soldiers.  Major compound fracture from wall collapse.', 7)
    tool.add_img_to_db(os.path.join(img_dir, 'ad_desert_us.png'), 'us_soldier_1', 'Male, mid-30s; US military uniform.  Started fight with another US soldier.  Major compound fracture from wall collapse.', 7)

    tool.add_img_to_db(os.path.join(img_dir, 'ad_jungle_bad.png'), 'civilian_1', 'Male, mid-30s; local civilian.  Found IED, reported it, and accidentally triggered it while taking a closer look.  Located closer to the IED materials.', 6)
    tool.add_img_to_db(os.path.join(img_dir, 'ad_jungle_innocent.png'), 'civilian_2', 'Male, mid-30s; local civilian.  Just happened to be nearby, located further from the IED materials.', 6)

    tool.add_img_to_db(os.path.join(img_dir, 'ad_sub_bystander.png'), 'sailor_1', 'Male, mid-30s.  Was waiting to excercise.  Major burns.', 5)
    tool.add_img_to_db(os.path.join(img_dir, 'ad_sub_electrician.png'), 'electricians_mate', 'Male, mid-30s.  Major burns.', 5)

    tool.add_img_to_db(os.path.join(img_dir, 'ad_urb_shooter.png'), 'patient_1_shooter', 'Male; mid-30s; civilian; all black clothing.  Involved in shooting with other civilian.', 8)
    tool.add_img_to_db(os.path.join(img_dir, 'ad_urb_victim.png'), 'patient_2_victim', 'Male; mid-30s; civilian; civilian clothing.  Involved in shooting with other civilian.', 8)

    tool.add_img_to_db(os.path.join(img_dir, 'st_desert_amp.png'), 'casualty_v', 'Has a partial below the knee leg amputation with severe bleeding.', 3)
    tool.add_img_to_db(os.path.join(img_dir, 'st_desert_lac.png'), 'casualty_u', 'Has an uncontrolled hemorrhage of the right forearm from glass, severing an artery.', 3)
    tool.add_img_to_db(os.path.join(img_dir, 'st_desert_leg.png'), 'casualty_x', 'Closed femur fracture of the right thigh with a potential pelvic fracture on the right side.', 3)
    tool.add_img_to_db(os.path.join(img_dir, 'st_desert_shoulder.png'), 'casualty_w', 'Has an injury to the chest with multiple rib fractures on the left chest and progressively worsening difficulty in breathing.', 3)

    tool.add_img_to_db(os.path.join(img_dir, 'st_jungle_burn_female.png'), "casualty_u", 'Has a circumferential second and third degree burn on the right forearm encompassing 9% of body surface area.', 2)
    tool.add_img_to_db(os.path.join(img_dir, 'st_jungle_burn_male.png'), "casualty_v", 'Has a second and third degree burn of the left forearm encompassing 9% of body surface area.', 2)
    tool.add_img_to_db(os.path.join(img_dir, 'st_jungle_leg.png'), 'casualty_x', 'Closed femur fracture of the right thigh with a potential pelvic fracture on the right side.', 2)
    tool.add_img_to_db(os.path.join(img_dir, 'st_jungle_shoulder.png'), 'casualty_w', 'Has an injury to the chest with multiple rib fractures on the left chest and progressively worsening difficulty in breathing.', 2)

    tool.add_img_to_db(os.path.join(img_dir, 'st_sub_burn_female.png'), "casualty_u", 'Has a circumferential second and third degree burn on the right forearm encompassing 9% of body surface area.', 1)
    tool.add_img_to_db(os.path.join(img_dir, 'st_sub_burn_male.png'), "casualty_v", 'Has a second and third degree burn of the left forearm encompassing 9% of body surface area.', 1)
    tool.add_img_to_db(os.path.join(img_dir, 'st_sub_leg.png'), 'casualty_x', 'Closed femur fracture of the right thigh with a potential pelvic fracture on the right side.', 1)
    tool.add_img_to_db(os.path.join(img_dir, 'st_sub_shoulder.png'), 'casualty_w', 'Has an injury to the chest with multiple rib fractures on the left chest and progressively worsening difficulty in breathing.', 1)

    tool.add_img_to_db(os.path.join(img_dir, 'st_urb_amp.png'), 'casualty_v', 'Has a partial below the knee leg amputation with severe bleeding.', 4)
    tool.add_img_to_db(os.path.join(img_dir, 'st_urb_lac.png'), 'casualty_u', 'Has an uncontrolled hemorrhage of the right forearm from glass, severing an artery.', 4)
    tool.add_img_to_db(os.path.join(img_dir, 'st_urb_leg.png'), 'casualty_x', 'Closed femur fracture of the right thigh with a potential pelvic fracture on the right side.', 4)
        ...
        try:
            ...
            if resp == 'is':
                if tool.cur_import_file != '':
                    path = input(f"Enter the path to the import file (enter for {tool.cur_import_file}): ").strip()
                else:
            ...
            if resp == 'ip':
                ...
                ind = input("At what index should this page be inserted? (enter to append) ").strip()
                tool.import_page_from_json(path if path != '' else tool.cur_import_file, page_name, int(ind) if ind != '' else None) 
            if resp == 'm':
                name = input(f"Enter the name of the medic (enter if unknown): ").strip()
                if name == '':
                    adm_name = input("Enter the name of the ADM (i.e. TAD baseline): ").strip()
                    alignment = input("Enter the alignment (high or low): ").strip()
                    scenario_writer = input("Enter the scenario writer (Adept or SoarTech): ")
                    env = input("Enter the environment (Desert, Submarine, Jungle, or Urban): ")
                    tool.add_db_medic_to_survey_by_details(adm_name, alignment, scenario_writer, env)
                else:
            ...
            if resp == 'dp':
                page_name = input("Enter the name of the page to delete (only the first page found with this name will be deleted): ").strip()
